...
void main() {
  test('serves a bounded block, re-advertises range, releases upstream', () async {
    ...
    expect(res.headers.value(HttpHeaders.contentRangeHeader), 'bytes 10-99/100');
  ...
  test('serves repeated reads inside one block from cache (no extra upstream)', () async {
    final data = Uint8List.fromList(List.generate(100, (i) => i));
    final upstream = await _serveBuffer(data);
    addTearDown(() => upstream.server.close(force: true));
  ...
  test('fetches a second block when the read crosses the grid boundary', () async {
