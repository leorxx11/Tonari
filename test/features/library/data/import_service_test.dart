...
void main() {
  ...
  test('each audio file becomes its own track (no quality merging)', () async {
    final summary = await service.applyScanResult(
      ScanResult(
        rootPath: '/scan',
        filesScanned: 2,
  ...
  test('persists sourceFolderId on insert and update', () async {
    final scan = ScanResult(
      ...
      works: [
        _work(
          rj: 'RJ_SRC',
          rootPath: '/scan/RJ_SRC',
          audios: [
            _audio(
              path: '/scan/RJ_SRC/t.mp3',
              fileName: 't.mp3',
              format: 'mp3',
            ),
          ],
    ...
    var work = await (db.select(
      db.works,
    )..where((w) => w.productId.equals('RJ_SRC'))).getSingle();
  ...
  test('applyScanResult clears needsRescan flag on updated works', () async {
    ...
    expect(
      (await (db.select(
        db.works,
      )..where((w) => w.productId.equals('RJ_FLAG'))).getSingle()).needsRescan,
      isTrue,
    );
