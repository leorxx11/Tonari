...
**目标**：用户自配 LLM API 翻译字幕，质量优于传统翻译 API。
...
- **翻译策略**：
  - 字幕按 20-30 行打包翻译（保上下文）
  - 仅翻译文本，保留时间码不变
  - JSON 输入输出格式（每行有 id，便于回填）
  - System Prompt 中明确：保留拟声词原文、称呼按日语原意保留、传达语气词不直译
- **错误处理**：
  - 网络失败、API 限流、JSON 解析失败均有友好提示
...
| **M6: LLM 翻译** | Provider 配置 + 字幕翻译 + 缓存 | DeepSeek 翻译一部完整字幕，缓存生效 |
