...
一款 iOS 端 ASMR 播放器，按 DLsite 作品（RJ 编号）组织音视频资源。核心特性：

- 本地音视频文件管理，按 RJ 编号识别 DLsite 作品
...
- ❌ 不做永久下载到本地（远程作品走流播 + 块缓存，不做整文件落盘 pin）

> 注：原「不做 NAS / SMB / WebDAV 远程音频」一条已作废 —— 云端片库已成为核心能力，见 §3.8 与 §10。
...
- **手动加载**：允许用户为某个音频文件手动指定字幕路径（**暂未实现**，播放页「选择字幕」入口当前为占位）
- **显示控制**：
  - 字号、行距、颜色、阴影/描边自定义
  - 字幕在播放页中央滚动区显示，当前行高亮
...
**目标**：iPhone 本地存储有限，把 ASMR 收藏放云端、手机当播放终端。远程作品在媒体库里与本地作品同样有 RJ 卡片、元数据、track 列表。

**Provider**：
- **WebDAV 直连**：用户自有 WebDAV 服务器（NAS / PikPak-over-WebDAV 等），标准 PROPFIND，密码进 Keychain
- **115 网盘**：TV 端二维码扫码登录拿 cookie → 浏览 + 流播。无官方 WebDAV，OpenAPI 个人开发者审核关停，cookie 路线实测可行

**导入与生命周期**：
- 远程目录结构与本地同构（大文件夹 → RJ 子文件夹），**快照式**导入；扫描阶段即跳过已导入作品（解决 115 限流锁死、可续扫）
- 导入管线与本地几乎原样复用：扫描产出同样的 `ScanResult`，元数据补全走后台 `EnrichmentQueue`（串行、限重试）
...
| **M9: 云端片库 + 视频** | WebDAV / 115 远程作品导入与流播、fvp 视频播放 | 远程作品像本地一样浏览/播放，11GB 大文件可流播，MKV/HEVC 可放 |
...
> 原为「待讨论方案（未锁定）」，2026-06 已实现并真机验证，功能详见 §3.8。此处保留关键决策与放弃项备查。
...
- **115 限并发**：同一签名直链约 2 条连接，超出 `403 115 pmt`。代理加并发信号量、按 4MB 块缓存 + 有限块响应，命中率约 98%，上游请求降两个数量级（见 §3.8）。
- **115 字幕下载**：必须走整文件直连下载，不能经流播代理（代理只回有限块 206 会被当下载失败）。
- **远程源失败反馈**：离线 / 鉴权失效时弹 SnackBar，连接错误映射为中文文案。

### 10.3 遗留项

- 媒体来源页收敛为「查看 + 删除」，刷新 = 重新导入。
- 已导入但缺元数据 / 字幕的作品不会自动补全，需走详情页「重新扫描此作品」。
