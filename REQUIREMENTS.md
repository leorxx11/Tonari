...
> 此节为探索项，未进入正式路线图。决策未定前不动手实现；M6 简介翻译 + M7 隐私功能完成后重新评估启动时机。

### 10.1 云盘存储集成（115 / PikPak）

**动机**：iPhone 本地存储有限，用户的 ASMR 收藏体量远超手机容量；用户持有 115 网盘 40TB 会员 + PikPak 会员，希望把手机当播放终端、网盘当片库。

**与 §2.3 的冲突**：现有约束明确「❌ 不做 NAS / SMB / WebDAV 远程音频」。一旦启动，§2.3 该条要删除或改写。

**候选 Provider**：
- **PikPak**：官方 WebDAV，用户已有会员（无限速顾虑），标准协议、代码简洁。海外服务，国内连通性看运营商
- **115**：无官方 WebDAV，走 OpenAPI（用户开发者申请审核中）。国内 CDN 直连快，但风控敏感、签名直链 2 小时过期

**目标形态**：两个 Provider 都接，App 内统一抽象。预期用户主用 115，PikPak 兼容备用

**关键技术决策点（启动时再定）**：
1. **下载模式 vs 流播模式**：先做下载（按需拉到 sandbox，播放体验完全不变），还是直接做流播（链接续期、错误重连、字幕图片单独下载、LRU 缓存全套）
2. **抽象层位置**：`MediaSource` + `CloudProvider` 两层抽象。`Track.filePath` 字段保留向后兼容，新增 `sourceType` + `remoteRef` 列
3. **离线兜底**：无论选哪种模式，章节级 LRU 缓存 + 手动 pin 是必须项
4. **iOS 后台下载**：原生 `URLSession.background` channel 还是前台

**初步阶段划分（粗估，待启动时细化）**：

| 阶段 | 内容 | 工作量 |
|---|---|---|
| C1 | MediaSource / CloudProvider 抽象层，本地实现迁移（行为不变）+ Provider 配置页 | 3-4 天 |
| C2 | PikPak WebDAV：列目录 + 一键下载到 sandbox | 3-4 天 |
| C3 | 115 OpenAPI：列目录 + 一键下载（等用户开发者审核通过） | 4-5 天 |
| C4 | 切换到真·流播：链接续期、错误重连、字幕图片下载、LRU 缓存、离线降级 | 5-7 天 |

C1+C2+C3 完成即获得「远程片库 + 本地按需下载」能力。C4 是否做按届时实际体验决定。

**未决问题**：
- 115 OpenAPI 对 R18 内容路径是否有额外策略限制（未知）
- PikPak 海外服务的国内长期可用性（移动网络下不稳定）
- REQUIREMENTS.md §2.3 改动时机：正式启动 C1 前更新
